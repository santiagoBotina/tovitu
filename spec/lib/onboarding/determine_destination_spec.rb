...
RSpec.describe Onboarding::DetermineDestination do
  describe "#call" do
    context "with individual user" do
      let(:individual) { create(:user, :verified, role: "individual") }

      it "returns locale-scoped onboarding path when not completed" do
        result = described_class.call(user: individual)
        expect(result).to eq("/en/onboarding/individual/questions")
      end

      it "returns pets path when completed" do
        individual.update!(onboarding_completed_at: Time.current)
        result = described_class.call(user: individual)
        expect(result).to eq("/en/pets")
      end
    end
    ...
    context "with shelter user" do
      ...
      it "returns locale-scoped onboarding path when not completed" do
        ...
        expect(result).to eq("/en/onboarding/shelter/questions")
      end
    ...
    context "with admin role" do
      ...
      it "returns shelter onboarding path (admin falls under shelter_user)" do
