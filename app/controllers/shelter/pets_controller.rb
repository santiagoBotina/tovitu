module Shelter
  class PetsController < ApplicationController
    ...
    def create
      ...
      if result.success?
        redirect_to shelter_pet_path(result.data), notice: t("pets.notices.created")
      else
    ...
    def update
      ...
      if result.success?
        redirect_to shelter_pet_path(@pet), notice: t("pets.notices.updated")
      else
        flash.now[:alert] = Array(result.errors).join(", ")
    ...
    def change_status
      ...
      if result.success?
        redirect_to shelter_pet_path(@pet), notice: t("pets.notices.status_changed", status: t("pets.status.#{result.data.status}"))
      else
